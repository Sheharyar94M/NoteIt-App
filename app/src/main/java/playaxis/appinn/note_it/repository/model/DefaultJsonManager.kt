...
package playaxis.appinn.note_it.repository.model
...
class DefaultJsonManager @Inject constructor(
    ...
    override suspend fun exportJsonData(): String {
        ...
        for (noteWithLabels in notesList) {
            ...
            notesMap[note.id] = NoteSurrogate(note.type, note.folderId!!, note.title, note.content,note.colorNote,note.isDrafted,"",
                "","",
                NoteTextFormat(
                    spannable = false,
                    fontPosition = -1
    ...
    override suspend fun importJsonData(data: String, importKey: SecretKey?): JsonManager.ImportResult {
        ...
        val jsonData: String = try {
            ...
            if (importKey == null) {
                ...
                return JsonManager.ImportResult.KEY_MISSING_OR_INCORRECT
            }
            ...
            val plaintext = try {
                ...
                return JsonManager.ImportResult.BAD_DATA
            }
            plaintext
        } catch (e: Exception) {
        ...
        val notesData: NotesData = try {
            ...
            return JsonManager.ImportResult.BAD_FORMAT
        }
        ...
        return if (notesData.version > VERSION) {
            ...
            JsonManager.ImportResult.FUTURE_VERSION
        } else {
    ...
    private suspend fun importNotes(notesData: NotesData, newLabelsMap: Map<Long, Long>) {
        ...
        for ((id, ns) in notesData.notes) {
            var noteId = id
            val newNote = Note(id, ns.folderId,ns.type, ns.title, ns.content, ns.colorNote,ns.isDrafted,"",
                "", images,
                NoteTextFormat(
                    spannable = false,
                    fontPosition = -1
                ),
                ns.metadata, ns.addedDate,
            ...
            when {
                ...
                oldNote.note.addedDate == newNote.addedDate && oldNote.note.lastModifiedDate == newNote.lastModifiedDate -> {
...
//    private suspend fun importFolders(folderNote: FolderNote, newLabelsMap: Map<Long, Long>) {
//        val existingNotes = notesDao.getAll().associateBy { it.note.id }
//        val labelRefs = mutableListOf<LabelRef>()
//
//        for ((id, ns) in notesData.notes) {
//            var noteId = id
//            val newNote = Note(id, ns.type, ns.title, ns.content, ns.colorNote,ns.metadata, ns.addedDate,
//                ns.lastModifiedDate, ns.status, ns.pinned, ns.reminder)
//            val oldNote = existingNotes[noteId]
//
//            // Remap labels appropriately and discard unresolved label IDs.
...
//    }
    ...
    private fun mergeNotes(old: Note, new: Note): Note? {
        val reminder = when {
            ...
            old.reminder != null && new.reminder != null && !compareReminders(old.reminder, new.reminder) -> {
