...
class ColorsListAdapter(private var itemClicked: ColorItemCLickEvent): RecyclerView.Adapter<RecyclerView.ViewHolder>() {

    private var colorList: ArrayList<ColorNote> = ArrayList()
    private val VIEW_TYPE_FIRST_ITEM = 0
    private val VIEW_TYPE_REGULAR_ITEM = 1

    private var selectedItemPosition = 0

    fun setColorList(colorList: ArrayList<ColorNote>){
    ...
    override fun onBindViewHolder(holder: RecyclerView.ViewHolder, position: Int) {
        ...
        if (holder is FirstItemViewHolder) {
            ...
            changeItemBackground(holder, colorList[position].isSelected && selectedItemPosition == position)
        }
        else if (holder is RegularItemViewHolder){
            // Bind data for regular items
            holder.itemColor.setImageResource(0)
            holder.itemColor.setBackgroundColor(colorList[position-1].color.toInt())
            holder.cardItem.strokeColor = ContextCompat.getColor(QuickNotepad.appContext,R.color.white)

            //change background of clicked item
            changeItemBackground(holder, colorList[position-1].isSelected && selectedItemPosition == position)
        }
        ...
        holder.itemView.setOnClickListener {
            ...
            if (holder is FirstItemViewHolder) {
                ...
                if (selectedItemPosition != item_position) {
                    ...
                    itemClicked.colorItemClicked(0,item_position)
                }
                ...
                if (selectedItemPosition != item_position) {
                    selectedItemPosition = item_position
                    itemClicked.colorItemClicked(colorList[position-1].color.toInt(),item_position)
                }
            }
        }
    }
    ...
    interface ColorItemCLickEvent{
        fun colorItemClicked(color: Int, item_position: Int)
    }
}