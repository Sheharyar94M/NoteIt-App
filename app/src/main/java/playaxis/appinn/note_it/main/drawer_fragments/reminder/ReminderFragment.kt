package playaxis.appinn.note_it.main.drawer_fragments.reminder
...
import playaxis.appinn.note_it.databinding.FragmentReminderBinding
...
class ReminderFragment : Fragment() {
    ...
    private fun noteListCommitCallback() {
        ...
        if (currentHomeDestinationChanged) {
            if (binding.reminderList.adapter!!.itemCount > 0) {
                binding.reminderList.scrollToPosition(0)
                binding.reminderList.scrollBy(0, -1)
            }
    ...
    private fun observers(adapter: NoteAdapter, layoutManager: StaggeredGridLayoutManager){
        ...
        viewModelHome.editItemEvent.observeEvent(viewLifecycleOwner) { note, pos ->
            ...
            val itemView: View = binding.reminderList.findViewHolderForAdapterPosition(pos)!!.itemView.findViewById(R.id.card_view)
...
//                binding.reminderList.scrollToPosition(pos)
//            }
            ...
            if (note.type == NoteType.TEXT)
                navController.navigateSafe(HomeFragmentDirections.homeToAddEditNoteFragment(note.id), extras = extras)
            else if (note.type == NoteType.LIST)
                navController.navigateSafe(HomeFragmentDirections.homeToTodoFragment(note.id), extras = extras)
        }
        ...
        viewModelHome.placeholderData.observe(viewLifecycleOwner) { data ->
            if (data != null) {
                ...
                binding.reminderList.layoutManager = StaggeredGridLayoutManager(spanCount, StaggeredGridLayoutManager.VERTICAL)
            }
        ...
        viewModelHome.noteCreatedEvent.observeEvent(viewLifecycleOwner) { noteId ->
            ...
            OneShotPreDrawListener.add(binding.reminderList) {
                exitTransition = null
                ...
                for (c in binding.reminderList.children) {
